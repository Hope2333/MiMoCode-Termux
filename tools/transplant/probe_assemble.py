...
def _resolve_bun_url() -> str:
    """Resolve the android bun base URL from config/bun-bind.json
    (target version + url_pattern). MiMoCode port: section-format module
    graphs require a >=1.4 base, so the bind config (target=1.4.0) wins over
    the legacy hardcoded BUN_URL fallback below."""
    cfg = Path(__file__).resolve().parent / "config" / "bun-bind.json"
    try:
        bind = json.loads(cfg.read_text(encoding="utf-8"))
        return str(bind["url_pattern"]).format(ver=bind["target"])
