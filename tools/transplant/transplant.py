...
Output: artifacts/transplant/<ver>/<product>-native + report.json
...
Revived binary: artifacts/transplant/<ver>/<product>-native-revived
(standalone mode: --version prints the product version, e.g. 1.3.13).

MiMoCode adaptation (ported from opencode-termux): --member selects the
tarball ELF member (default "/bin/opencode" suffix match preserved; for the
mimo tarball pass --member mimo). --product overrides the output file stem
(default "opencode", derived from --member otherwise; MiMoCode builds use
--product mimocode -> mimocode-native-revived).
...
def product_stem(member: str | None, product: str | None) -> str:
    """Output file stem: 'opencode' by default; --product overrides, else
    derived from --member's basename (e.g. --member mimo -> 'mimo')."""
    if product:
        return product
    m = (member or DEFAULT_MEMBER).lstrip("/")
    return "opencode" if m == "bin/opencode" else Path(m).name

def member_matches(name: str, member: str) -> bool:
    """Tarball ELF member match: exact path or '/<member>' suffix.
    Default member '/bin/opencode' preserves the historical endswith check."""
    needle = member.lstrip("/")
    return name == needle or name.endswith("/" + needle)

def _product_of(args) -> str:
    """Product stem for a parsed argparse namespace (attr-tolerant)."""
    return product_stem(getattr(args, "member", None), getattr(args, "product", None))
...
# ---------------------------------------------------------------- extract
def extract_step(tgz: Path, ver: str, out_dir: Path, member: str = DEFAULT_MEMBER) -> dict:
    """tgz -> module-graph.bin + host-bun.bin (probe_extract core logic).

    member: tarball ELF member selector (exact path or '/<name>' suffix;
    default '/bin/opencode'; MiMoCode tarballs carry a root-level 'mimo')."""
    if not tgz.is_file():
    ...
    with tarfile.open(tgz, "r:gz") as tf:
        for m in tf.getmembers():
            if m.isfile() and member_matches(m.name, member):
                elf_member = m
                break
        if elf_member is None:
            raise TransplantError(f"no member matching {member!r} in {tgz}")
        ...
        if ver is not None and not selected:
            if matched:
                info["warnings"].append(
                    f"version {ver} matched patch range(s) with empty patch "
                    f"lists; patch step no-op"
                )
            else:
                info["warnings"].append(
...
def revive_step(bun: Path, graph: Path, out: Path) -> dict:
    ...
    # revive_patch.py success markers: reloc mode prints the extended rela
    # table line "...(n=N) += RELATIVE off=... addend=..."; plain-offset mode
    # (bun >=1.4 bases) instead prints "plain-offset 0x<vaddr>" and touches no
    # RELA table. Accept either.
    m = re.search(r"\(n=(\d+)\) \+= RELATIVE", proc.stdout)
    if m:
        info["reloc_count"] = int(m.group(1))
        info["size_mode"] = "reloc"
    else:
        m2 = re.search(r"plain-offset (0x[0-9a-fA-F]+)", proc.stdout)
        if not m2:
            raise TransplantError(
                f"revive surgery succeeded but neither rela-table nor "
                f"plain-offset marker found in output; stdout:\n{proc.stdout}"
            )
        info["size_mode"] = "plain-offset"
        info["size_field_vaddr"] = int(m2.group(1), 16)
...
def cmd_extract(args) -> int:
    ...
    info = extract_step(tgz, args.ver, out_dir, member=getattr(args, "member", DEFAULT_MEMBER))
...
def cmd_detect(args) -> int:
    ...
    if args.binary:
        ...
        src = out_dir / f"{_product_of(args)}-native"
    if not src.is_file():
...
def cmd_assemble(args) -> int:
    ...
    graph = Path(args.graph) if args.graph else out_dir / "module-graph.bin"
    out_path = Path(args.out) if args.out else out_dir / f"{_product_of(args)}-native"
    if not graph.is_file():
        die(f"module graph not found: {graph} (run extract first)")
...
def cmd_verify(args) -> int:
    out_dir = ver_dir(args.ver)
    binary = Path(args.binary) if args.binary else out_dir / f"{_product_of(args)}-native"
...
def cmd_revive(args) -> int:
    ...
    graph = Path(args.graph) if args.graph else out_dir / "module-graph.bin"
    out = Path(args.out) if args.out else out_dir / f"{_product_of(args)}-native-revived"
...
def cmd_all(args) -> int:
    ...
    # 1. extract
    print("== extract ==")
    extract_info = extract_step(tgz, ver, out_dir, member=getattr(args, "member", DEFAULT_MEMBER))
    ...
    stem = _product_of(args)
    out_path = out_dir / f"{stem}-native"
    bun_elf = None
    if section_mode:
        ...
        print(f"  {stem}-native sha256={assemble_info['sha256'][:16]}...")


    # 6. revive (C1 revival surgery; skip with --no-revive)
    revived_path = out_dir / f"{stem}-native-revived"
    revived_ok = False
    if args.no_revive:
        ...
        revived_ok = True
        print(f"  {stem}-native-revived sha256={revive_info['sha256'][:16]}... "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--member", default=DEFAULT_MEMBER, help="tarball ELF member to extract (exact path or '/<name>' suffix; default: /bin/opencode; MiMoCode: --member mimo)")
    p.add_argument("--product", default=None, help="output file stem (default: derived from --member; MiMoCode: --product mimocode)")
